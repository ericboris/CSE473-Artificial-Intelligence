...
def MDP_command(cmd, param):
    ...
    if cmd == "show_values":
        ...
        if param == 2:
            Vis.show_q_values(Q_from_VI, CLOSED)  # Vis.reshow_all_q_values(Q_from_VI, CLOSED)
            update_policy_displays(which="VI")
        ...
        if param == 4:
            Vis.show_q_values(Q_from_QL, CLOSED)  # Vis.reshow_all_q_values(Q_from_QL, CLOSED)
            update_policy_displays(which="QL")
        return
