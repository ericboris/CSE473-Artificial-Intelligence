...
class Tracker(object):
    def __init__(self, questions, maxes, prereqs, mute_output, gs_output=False):
        ...
        print('\nStarted at %d:%02d:%02d' % time.localtime()[3:6])
...
def verify_node(node, expected_type, expected_shape, method_name):
    ...
    if expected_type != 'loss':
        assert all(
            [(expected == '?' or actual == expected) for (actual, expected) in
...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        ...
        for point in (random_point, np.zeros_like(random_point)):
            ...
            expected_prediction = np.where(np.dot(
                point, p.get_weights().data.T) >= 0, 1, -1).item()
    ...
    accuracy = np.mean(
        np.where(np.dot(dataset.x, model.get_weights().data.T) >= 0.0, 1.0,
                 -1.0) == dataset.y)
    if accuracy < 1.0:
        print(
    ...
    for batch_size in (1, 2, 4):
        ...
        for node in trace:
            assert not isinstance(node,
                                  nn.Parameter) or node in detected_parameters, (
                "Calling RegressionModel.run() multiple times should always "
                "re-use the same parameters, but a new nn.Parameter object was "
                "detected")
            if isinstance(node, nn.ReLU) or isinstance(node, nn.Tanh):
                found_nonlinearity = True
            if isinstance(node, nn.AddBias):
                found_bias = True
    assert found_nonlinearity, (
        "Node returned from RegressionModel.run() does not "
        "depend on any nonlinearity, your neural network is incorrect.")
    if not found_bias:
        print("WARNING: Node returned from RegressionModel.run() does not"
              "depend on any bias, your code probably has a bug!")
...
def check_digit_classification(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        for node in trace:
            assert not isinstance(node,
                                  nn.Parameter) or node in detected_parameters, (
                "Calling DigitClassificationModel.run() multiple times should "
            ...
            if isinstance(node, nn.AddBias):
                found_bias = True
    assert found_nonlinearity, (
        "Node returned from DigitClassificationModel.run() does not "
        "depend on any nonlinearity, your neural network is incorrect.")
    if not found_bias:
        print("WARNING: Node returned from DigitClassificationModel.run() does "
              "not depend on any bias, your code probably has a bug!")
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        if word_length > 1:
            ...
            for node in trace:
                assert not isinstance(node,
                                      nn.Parameter) or node in detected_parameters, (
                    "Calling LanguageIDModel.run() multiple times should "
                    "always re-use the same parameters, but a new nn.Parameter "
                    "object was detected")
                if isinstance(node, nn.ReLU) or isinstance(node, nn.Tanh):
                    found_nonlinearity = True
                if isinstance(node, nn.AddBias):
                    found_bias = True
    assert found_nonlinearity, (
        "Node returned from LanguageIDModel.run() does not "
        "depend on any nonlinearity, your neural network is incorrect!")
    if not found_bias:
        print("WARNING: Node returned from LanguageIDModel.run() does not"
              "depend on any bias, your code probably has a bug!")
